...
app.get("/random",  (req, res) => {
    ...
    const result = jokes[Math.round(Math.random()*101)];
    console.log(result)
    res.json(result);
});
...
app.get("/jokes/:id",  (req, res) => {
  ...
  const jk = jokes.find((joke) => joke.id === id);
  res.json(jk);
});
...
app.get("/filter",  (req, res) => {
  ...
  const jk = jokes.filter((joke) => joke.jokeType === type);
  res.json(jk);
});
...
app.delete("/jokes/:id",  (req, res) => {
  const id = parseInt(req.params.id)
  let index = jokes.findIndex((obj) => obj.id === id);
  if (index !== -1) {
